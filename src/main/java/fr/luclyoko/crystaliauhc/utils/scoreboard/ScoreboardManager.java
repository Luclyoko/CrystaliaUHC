...
public class ScoreboardManager {
    ...
    public void onDisable() {
        if (scoreboards.values().size() != 0) scoreboards.values().forEach(PersonalScoreboard::onLogout);
    }
    ...
    private String colorIpAt() {
        ...
        if (ipCharIndex + 1 < ip.length()) {
            ...
            if (ipCharIndex + 2 < ip.length())
                formattedIp.append(ChatColor.DARK_AQUA).append(ip.substring(ipCharIndex + 2));
        ...
        return ChatColor.DARK_AQUA + formattedIp.toString();
    }
