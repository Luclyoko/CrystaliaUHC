...
public class KiraDeathNote extends ArenaPower {
    public KiraDeathNote(ArenaUHC arenaUHC, ArenaRole arenaRole, Main main) {
        super(arenaUHC, arenaRole, main);
        this.arenaPowerEnum = ArenaPowerEnum.KIRA_DEATH_NOTE;
        this.cooldown = 180;
        this.duration = 60;
    }
        ...
        if (nearestPlayer.isPresent()) {
            ...
            if (crystaliaPlayer.getRole() != null && crystaliaPlayer.getRole() instanceof ArenaRole) {
                ...
                if (user.getRole() instanceof ArenaRole) {
                    ...
                    user.getRole().setCanSeeLife(true);
                    Bukkit.getScheduler().runTaskLater(main, () -> {
                        if (user.getRole() != null && user.getRole().equals(role))
                            user.getRole().setCanSeeLife(false);
                    }, duration * 20L);
                }
            }
        } else {
    ...
    public String getDescription() {
        return "Vous permet de retirer §c3❤ permanents §fau joueur que vous visez ainsi que de §bvoir la vie §fpendant 1 minute.";
    }
}
