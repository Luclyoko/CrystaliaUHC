...
public class LGRolesManager implements Timer {
    ...
    public LGRolesManager(Main main, GameManager gameManager, int rolesTime) {
        ...
        this.remindTimers = Arrays.asList(60, 5);
        this.roles = false;
    ...
    public void init(boolean sound) {
        ...
        for (LGRolesEnum lgRolesEnum : this.composition.keySet().stream().filter(lgRolesEnum -> (this.composition.get(lgRolesEnum) > 0)).collect(Collectors.toList())) {
            for (int i = 1; i <= this.composition.get(lgRolesEnum); i++) {
                if (!gamePlayers.isEmpty()) {
                    CrystaliaPlayer player = gamePlayers.get(0);
                    try {
                        player.setRole(lgRolesEnum.getRoleClass().getDeclaredConstructor(classArgs).newInstance(this.gameManager, player));
