...
public class LGCompoHybridesGui implements GuiBuilder {
    public String name() {
        return "§dComposition des Hybrides";
    }
    ...
    public void contents(Player player, Inventory inv) {
        for (int i = 0; i < getSize(); i++) {
            if ((i <= 8 || i >= getSize() - 9 || i % 9 == 0 || i % 9 == 8) && i !=
                    ...
                    getSize() - 1)
                inv.setItem(i, (new ItemBuilder(Material.STAINED_GLASS_PANE, (byte)2))
                        .setAmount(1)
                        .setDisplayName(" ")
                        .toItemStack());
        }
        LGUHC lguhc = (LGUHC)main.getGameManager().getGamemodeUhc();
        for (LGRolesEnum lgRolesEnum : lguhc.getRolesManager().getCompoForSide(LGSides.LGBasicSides.HYBRIDE).keySet().stream().sorted().collect(Collectors.toList())) {
